...
from dataset_vs import NiftiImageGenerator, NiftiPairImageGenerator
import argparse
import torch
...
parser.add_argument('-i', '--seg_folder', type=str, default="dataset/vs_seg/seg/")
parser.add_argument('-t1', '--t1_folder', type=str, default="dataset/vs_seg/t1/")
...
parser.add_argument('-r', '--resume_weight', type=str, default="model/model_vs.pt") #"model/model_brats.pt")
...
if with_condition:
    dataset = NiftiPairImageGenerator(
        ...
        full_channel_mask=True #use to prevent ordinal bias? 
    )
else:
    print("Please modify your code to unconditional generation")


in_channels = 3 #  3 (mask) one hot encoding (background 0, head 1, tumor 2)
out_channels = 1 # 1 bc produce just T1 image
