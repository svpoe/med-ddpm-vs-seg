...
def load_ras_float(path):
        img = nib.load(path)
        img = nib.as_closest_canonical(img)
        return img.get_fdata().astype(np.float32, copy=False)
...
class NiftiPairImageGenerator(Dataset):
    ...
    def read_image(self, file_path, pass_scaler=False, per_volume=True):
        img = load_ras_float(file_path)
        if not pass_scaler:
            if per_volume:
                ...
                # keep your original per-depth behavior
    ...
    #REVISED TO KEEP MASKS BINARY
    def resize_img_4d(self, input_img):
        h, w, d, c = input_img.shape
        if h == self.input_size and w == self.input_size and d == self.depth_size:
            return input_img
        out = np.zeros((self.input_size, self.input_size, self.depth_size, c), dtype=input_img.dtype)
        rs = tio.Resize((self.input_size, self.input_size, self.depth_size), image_interpolation='nearest')
        for ch in range(c):
            ch_img = input_img[..., ch]
            tio_img = tio.LabelMap(tensor=ch_img[np.newaxis, ...])
            out[..., ch] = np.asarray(rs(tio_img))[0]
        # keep masks binary if using {0,1}
